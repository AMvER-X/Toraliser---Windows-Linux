...
{
    int out_index = 0;
    int jump = 0; // Flag: have we jumped using pointer?
    ...
    while (offset < packet_len)
    {
        ...
        // End of name, increase offset to move onto next stage
        if (len == 0)
        {
            offset++;
            break;
        ...
        if ((len & 0xC0) == 0xC0) // might need to be >= as it is 11 or greater as the most significant starting byte.
        {
            if (offset + 1 >= packet_len)
                return POINTER_COMPRESSION_LENGTH;
...
// Helper function to duplicate socket
int DuplicateSocket(SOCKET s, SOCKET *s2)
{
    WSAPROTOCOL_INFO protocolInfo;

    // Duplication of socket, store info in protocolInfo
    if (WSADuplicateSocket(s, GetCurrentProcessId(), &protocolInfo) != 0)
    {
        perror("WSADuplicateSocket failed with error: %d\n" + WSAGetLastError());
        return WSADUPSOCK;
    }

     // Create a new socket from duplicated socket info
     *s2 = WSASocket(AF_INET, SOCK_STREAM, IPPROTO_TCP, &protocolInfo, 0, 0);
     if (*s2 == INVALID_SOCKET)
     {
        perror("WSASocket (duplicate) failed with error: %d\n" + WSAGetLastError());
        return WSADUPSOCK;
     }
     return 0;
}
...
// Unix & Linux version
int connectW(SOCKET s2, const struct sockaddr *sock2, socklen_t addrlen)
{
    WSADATA wsaDATA;
    ...
    if (iResult != 0)
    {
        perror("WSAStartup failed with error code %d\n" + iResult);
        return WSASTARTUP;
    }

    // creation of function pointer for connect function, function hooking for the connect function
    int (*p)(int, const struct sockaddr *, socklen_t);

    // find next occurance of connect function
    p = (int (*)(int, const struct sockaddr*, socklen_t))GetProcAddress(GetModuleHandle("Ws2_32.dll"), "connect");
    if (!p)
    {
    ...
    // Prepare structure of the socket
    sock.sin_family = AF_INET;
    sock.sin_port = htons(TORPROXYPORT);  // Check if it needs to be DNS port
    ...
    // Extraction of target host and port
    struct sockaddr_in *target = (struct sockaddr_in *)sock2;
    inet_ntop(AF_INET, &target->sin_addr, host, sizeof(host)); // Convert binary IP to string
    port = ntohs(target->sin_port); // Convert port to host byte order
    ...
    // Sending of data
    if (send(s, (const void *)req, reqsize, 0) != reqsize)
    {
        perror("write");
        free(req);
        closesocket(s);
...
int TorDNSQuery(const char *hostname, struct sockaddr_in *resolved_addr)
{
    ...
    // creation of socket
    srand(time(NULL)); struct sockaddr_in torAddr;
    ...
    unsigned short transactionID = rand() & 0xFFFF;
    // Transaction ID
    ...
    */
    ...
    while (*hn)
    {
     ...
     // Skip question section, start pos 12
...
*/
int getaddrinfoW(const char *node, const char *service, const struct addrinfo *hints, struct addrinfo **res)
{
    //Check if the node is an IP address already, func returns none 0 if IPv4
    struct in_addr addr;
